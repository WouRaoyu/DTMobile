 ...
 * @ Modified time: 2024-07-14 18:14:06
 * @ Description:
 -->
...
<template>
    <van-pull-refresh v-model="loading" @refresh="onRefresh">
        <van-tabs style="margin-bottom: 64px;" v-model:active="active" scrollspy sticky>
            ...
            <van-tab title="围岩等级">
                <van-cell-group inset>
                    <van-cell :title=info.rockLevelDesc>
                    ...
                    <van-image width="100%" :src="'http://localhost:8000/' + info.rockLevelImg" />
                </van-cell-group>
            </van-tab>
            <van-tab title="破碎程度">
                <van-cell-group inset>
                    <van-cell :title=info.fragmentDesc>
                    ...
                    <van-image width="100%" :src="'http://localhost:8000/' + info.fragmentImg" />
                </van-cell-group>
            </van-tab>
            <van-tab title="坚硬程度">
                <van-cell-group inset>
                    <van-cell :title=info.hardnessDesc>
                    ...
                    <van-image width="100%" :src="'http://localhost:8000/' + info.hardnessImg" />
                </van-cell-group>
            </van-tab>
            <van-tab title="富水程度">
                <van-cell-group inset>
                    <van-cell :title=info.wateryDesc>
                        <van-tag v-if="info.watery == 0" color="rgb(220, 235, 250)">干燥</van-tag>
                        <van-tag v-if="info.watery == 1" color="rgb(142, 178, 241)">较干燥</van-tag>
                        <van-tag v-if="info.watery == 2" color="rgb(44, 108, 175)">较湿润</van-tag>
                        <van-tag v-if="info.watery == 3" color="rgb(0, 71, 147)">富水</van-tag>
                        <van-tag v-if="info.watery == 4" color="rgb(0, 15, 75)">极富水</van-tag>
                    </van-cell>
                    <van-image width="100%" :src="'http://localhost:8000/' + info.wateryImg" />
                </van-cell-group>
            </van-tab>
            <van-tab title="地应力状态">
                <van-cell-group inset>
                    <van-cell :title=info.geostressDesc>
                    ...
                    <van-image width="100%" :src="'http://localhost:8000/' + info.geostressImg" />
                </van-cell-group>
            </van-tab>
            <van-tab title="高清影像">
                <van-cell-group inset>
                    <van-cell :title=info.realSceneDesc></van-cell>
                    <van-image width="100%" :src="'http://localhost:8000/' + info.realSceneImg" />
                </van-cell-group>
            </van-tab>
        </van-tabs>
    </van-pull-refresh>
...
export default {
    setup() {
        ...
        const onRefresh = (first) => {
            axios.get('http://localhost:8000/judge_detail/' + dtId.value, { timeout: 5000 })
                .then(response => {
                    if (!first) {
                        Toast('刷新成功');
                    }
